...
class FleetRepository {
  ...
  final List<JetAdminModel> _inMemoryJets = [
    const JetAdminModel(
      ...
      seating: LocalizedText(
        en: 'SEATING : UP TO 4',
        fr: 'CAPACITÉ : JUSQU\'À 4',
        de: 'SITZPLÄTZE : BIS ZU 4',
        ar: 'المقاعد : حتى 4',
      ),
      range: LocalizedText(
        en: 'RANGE : UP TO 2.5 HOURS',
        fr: 'AUTONOMIE : JUSQU\'À 2,5 HEURES',
        de: 'REICHWEITE : BIS ZU 2,5 STUNDEN',
        ar: 'المدى : حتى 2.5 ساعة',
      ),
      description: LocalizedText(
        en: 'Very light jets (VLJs) are a cost-effective and flexible charter option for groups of up to four passengers. They have a lightweight airframe and impressive performance capabilities, allowing them to land in small airports with short runways.',
    ...
    const JetAdminModel(
      ...
      seating: LocalizedText(
        en: 'SEATING : UP TO 8',
        fr: 'CAPACITÉ : JUSQU\'À 8',
        de: 'SITZPLÄTZE : BIS ZU 8',
        ar: 'المقاعد : حتى 8',
      ),
      range: LocalizedText(
        en: 'RANGE : UP TO 4 HOURS',
        fr: 'AUTONOMIE : JUSQU\'À 4 HEURES',
        de: 'REICHWEITE : BIS ZU 4 STUNDEN',
        ar: 'المدى : حتى 4 ساعات',
      ),
      description: LocalizedText(
        en: 'Midsize jets offer the perfect balance of range, speed, and spacious cabin comfort for groups of up to eight passengers. Featuring generous stand-up headroom, ample luggage compartment capacity, and transcontinental capabilities.',
    ...
    const JetAdminModel(
      ...
      seating: LocalizedText(
        en: 'SEATING : UP TO 14',
        fr: 'CAPACITÉ : JUSQU\'À 14',
        de: 'SITZPLÄTZE : BIS ZU 14',
        ar: 'المقاعد : حتى 14',
      ),
      range: LocalizedText(
        en: 'RANGE : UP TO 8 HOURS',
        fr: 'AUTONOMIE : JUSQU\'À 8 HEURES',
        de: 'REICHWEITE : BIS ZU 8 STUNDEN',
        ar: 'المدى : حتى 8 ساعات',
      ),
      description: LocalizedText(
        en: 'Heavy jets represent the ultimate in private jet luxury and intercontinental performance. Designed to seat up to fourteen passengers across distinct cabin zones, with fully flat berthable seating and a dedicated attendant galley.',
    ...
    const JetAdminModel(
      ...
      range: LocalizedText(
        en: 'RANGE : UP TO 3 HOURS',
        fr: 'AUTONOMIE : JUSQU\'À 3 HEURES',
        de: 'REICHWEITE : BIS ZU 3 STUNDEN',
        ar: 'المدى : حتى 3 ساعات',
      ),
      description: LocalizedText(
        en: 'Imagine flying directly to your private jet by helicopter – no traffic jams, no detours, just take off and enjoy the world from above. Reach your destination quickly, comfortably, and in style.',
  ...
  void _initFirestoreListener() {
    try {
      _firestore.collection('jets').orderBy('order').snapshots().listen(
        (snapshot) {
        ...
        onError: (e) {
          dev.log('Firestore jets listener error: $e', name: 'FleetRepository');
          _notify();
        },
      );
    } catch (e) {
  ...
  Future<void> seedInitialJets({bool force = false}) async {
    try {
      ...
      await batch.commit();
      dev.log('Initial fleet catalog seeded successfully to Firestore', name: 'FleetRepository');
    } catch (e) {
      dev.log('Error seeding initial jets to Firestore: $e', name: 'FleetRepository');
    }
  }
  ...
  Future<JetAdminModel?> getJetById(String id) async {
    try {
      ...
      dev.log('Error fetching jet $id from Firestore: $e', name: 'FleetRepository');
    }
  ...
  Future<void> saveJet(JetAdminModel jet) async {
    final newId = jet.id.isNotEmpty ? jet.id : 'jet_${DateTime.now().millisecondsSinceEpoch}';
    final index = _inMemoryJets.indexWhere((j) => j.id == newId || (jet.id.isNotEmpty && j.id == jet.id));
    final finalOrder = jet.order > 0 ? jet.order : (index >= 0 ? _inMemoryJets[index].order : _inMemoryJets.length);
    final finalJet = jet.copyWith(
      id: newId,
      order: finalOrder,
      createdAt: jet.createdAt ?? (index >= 0 ? _inMemoryJets[index].createdAt : DateTime.now()),
    ...
    try {
      await _firestore.collection('jets').doc(newId).set(finalJet.toMap(), SetOptions(merge: true));
    } catch (e) {
      dev.log('Error saving jet $newId to Firestore: $e', name: 'FleetRepository');
      rethrow;
    }
  }
  ...
  Future<void> deleteJet(String id) async {
    ...
    try {
      ...
      dev.log('Error deleting jet $id from Firestore: $e', name: 'FleetRepository');
      rethrow;
    }
  }
  ...
  Future<void> toggleJetStatus(String id, bool isActive) async {
    ...
    if (index >= 0) {
      _inMemoryJets[index] = _inMemoryJets[index].copyWith(isActive: isActive, updatedAt: DateTime.now());
      _notify();
    }
    ...
    try {
      ...
      dev.log('Error toggling jet status in Firestore: $e', name: 'FleetRepository');
      rethrow;
    }
  }
  ...
  Future<void> updateJetsOrder(List<JetAdminModel> reorderedJets) async {
    ...
    try {
      ...
      dev.log('Error updating jets order in Firestore: $e', name: 'FleetRepository');
      rethrow;
    }
  }
}
